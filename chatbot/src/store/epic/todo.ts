...
export class TodoEpic {
    ...
    static markArchived = (action$) =>
        action$.ofType(TodoAction.MARK_TODO_ARCHIVED)
            .switchMap(({ payload }) => {
                return Observable.fromPromise(fs.todoref.child(payload.key + "/isDone").set(payload.isDone))
                    .map((x) => {
                        return { type: TodoAction.NULL };
                    })
            })
    ...
    static deleteTodo = (action$) =>
        action$.ofType(TodoAction.DELETE_TODO)
            .switchMap(({ payload }) => {
                return Observable.fromPromise(fs.todoref.child(payload.key).set(null))
                    .map((x) => {
                        return { type: TodoAction.NULL };
                    })
            })
